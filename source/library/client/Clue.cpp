...
#define TRACEOBJECT(TYPE, TYPEUPPERCASE) void TraceObject \
	( \
	ClueCategory	inCAT \
,	ClueEvent		inEVT \
,	uint32			inLineNumber \
...
{ \
	if ((CAM.m_data->m_port >= B_OK) && (CAM.m_data->m_mask[inCAT] & inEVT) && (CAM.m_data->m_mask[INDEX_##TYPEUPPERCASE] & MASK_##TYPEUPPERCASE)) /* do we pass the filter? */ \
	{ \
		thread_info TI; \
		get_thread_info (find_thread (NULL), &TI); \
		FingerPrint FP = {inCAT, inEVT, TI.thread, TI.team, real_time_clock_usecs (), inLineNumber, atomic_add (&Sequence, 1)}; \
 \
		BString strDetails ("Inspecting a " #TYPE ":\n"); \
		Inspect_##TYPE (strDetails, inValue, 0, "\n"); \
 \
		GatherDataAndSend (&FP, inFileName, inMethodDefinition, inDescription, strDetails); \
	} \
}

TRACEOBJECT(BMediaEncoder, BMEDIAENCODER)
TRACEOBJECT(BMultiChannelControl, BMULTICHANNELCONTROL)
TRACEOBJECT(BAlert, BALERT)
TRACEOBJECT(BAppFileInfo, BAPPFILEINFO)
TRACEOBJECT(BBitmap, BBITMAP)
TRACEOBJECT(BBitmapStream, BBITMAPSTREAM)
TRACEOBJECT(BString, BSTRING)
TRACEOBJECT(BBufferIO, BBUFFERIO)
TRACEOBJECT(BGameSound, BGAMESOUND)
TRACEOBJECT(BBox, BBOX)
TRACEOBJECT(BBuffer, BBUFFER)
TRACEOBJECT(BSmallBuffer, BSMALLBUFFER)
TRACEOBJECT(BBufferConsumer, BBUFFERCONSUMER)
TRACEOBJECT(BBufferGroup, BBUFFERGROUP)

#if !defined(CLUE_ON_ZETA_HACK) || defined(CLUE_FOR_ZETA_GL)
TRACEOBJECT(BGLView, BGLVIEW)
#endif
